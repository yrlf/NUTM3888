...
dat$EXLWMBC<-as.numeric(as.character(dat$EXLWMBC))
dat$EXLWVBC<-as.numeric(as.character(dat$EXLWVBC))
dat$EXLWTBC<-as.numeric(as.character(dat$EXLWTBC))

res<-randomForest(DIABBC~.-ABSPID, data=dat)
...
# ATTEMPT 3 (21/09/2021 Tue - 07/10/2021)
...
dat$bmi_level=as.factor(dat$bmi_level)
dat
```
...
```{r}
rp<-rpart(BMISC~SYSTOL+DIASTOL+TRIGRESB+FOLATREB+CHOLRESB+DIAHBRSK+HBA1PREB+GLUCFREB+ALTRESB,
      data = dat_age,
      #list(split="information"),
...
```
```{r}
save(dat_atmp3, file="data_12_18.Rdata")
```
